...
class TestLoginNegative:
    def test_empty_credentials_does_not_login(self, pages: Pages) -> None:
        ...
        assert not pages.auth.is_logged_in(timeout=3000), "Should NOT be logged in with empty credentials"
...
class TestLogout:
    def test_logout_removes_profile_link(self, auth_pages_ui: Pages) -> None:
        ...
        expect(auth_pages_ui.auth.page.locator("a[href*='/user']").first).not_to_be_visible(timeout=5000)
